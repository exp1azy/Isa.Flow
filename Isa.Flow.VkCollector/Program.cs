using Isa.Flow.VkCollector.Resources;
        ...
        static async Task Main(string[] args)
        {
            ...
            try
            {
                ...
                config = new ConfigurationBuilder()
                    .AddJsonFile("vkcollector.conf.json", optional: false, reloadOnChange: true)
                    .Build();
                Log.Logger.Information(Message.ConfigRead);

                Log.Logger.Information(Message.ActorInitializing);
                Console.CursorTop--;
                collector = new VkCollector(config);
