...
using Isa.Flow.TelegramCollector.EventArgs;
using Isa.Flow.TelegramCollector.Exceptions;
using WTelegram;
...
namespace Isa.Flow.TelegramCollector
{
    ...
    /// </summary>
    public class TgCollector : BaseActor
    {
            ...
            : base(new ConnectionFactory() { Uri = new Uri(config.GetSection("RabbitMq")["Uri"]!) }, config["ActorId"])
        {
            ...
            AddRpcHandler<StartTgCollectorRequest, TgCollectorResponse>(req =>
            {
                if (IsStarted)
                {
                    return new TgCollectorResponse { Message = "started", Status = TgCollectorStatusCode.OK };
                }

                phoneNumberEv.Reset();
                loginErrorEv.Reset();

                PhoneNumberRequested += PhoneNumberHandler;
                ...
                var endTask = Task.WhenAny(phoneNumberTask, loginErrorTask).Result;
                PhoneNumberRequested -= PhoneNumberHandler;
                LoginError -= LoginErrorHandler;
                ...
                if (ReferenceEquals(phoneNumberTask, endTask))
                {
                    ...
                    VerificationCodeRequested += VerificationHandler;
                    LoginError += LoginErrorHandler;

                    SetPhoneNumberAsync(req.Number).Wait();
                    ...
                    endTask = Task.WhenAny(startedTask, verificationTask, loginErrorTask).Result;
                    Started -= StartedHandler;
                    VerificationCodeRequested -= VerificationHandler;
                    LoginError -= LoginErrorHandler;
                    ...
                    {
                        return new TgCollectorResponse { Message = "verificationRequested", Status = TgCollectorStatusCode.OK };
                    }
                    else
                    {
                        return new TgCollectorResponse { Message = $"error: {loginException?.Message ?? "null"}{(loginException is RpcException rpce ? $" - {rpce.X}" : string.Empty)}",
                            Status = TgCollectorStatusCode.Error };
                    }                   
                }
                else
                {
            ...
            AddRpcHandler<SetTgCollectorVerificationRequest, TgCollectorResponse>(req =>
            {
                startedEv.Reset();
                loginErrorEv.Reset();
                ...
                _ = SetVerificationCodeAsync(req.VerificationCode);

                var loginErrorTask = Task.Run(loginErrorEv.WaitOne);
                var startedTask = Task.Run(startedEv.WaitOne);

                var endTask = Task.WhenAny(startedTask, loginErrorTask).Result;
                Started -= StartedHandler;
                LoginError -= LoginErrorHandler;

                return new TgCollectorResponse { Status = ReferenceEquals(endTask, startedTask) ? TgCollectorStatusCode.OK : TgCollectorStatusCode.Error };                   
            });

            AddRpcHandler<StopTgCollectorRequest, TgCollectorResponse>(req =>
            {
                StopAsync().Wait();

                return new TgCollectorResponse { Message = "stopped", Status = TgCollectorStatusCode.OK };
            });

            AddRpcHandler<TgCollectorCurrentStateRequest, TgCollectorCurrentStateResponse>(req => new TgCollectorCurrentStateResponse { IsStarted = IsStarted });
        }
        ...
        private void StartedHandler(object? sender, System.EventArgs e)
        {
            startedEv.Set();
        }

        private void VerificationHandler(object? sender, System.EventArgs e)
        {
            verificationEv.Set();
        }
        ...
        private void PhoneNumberHandler(object? sender, System.EventArgs e)
        {
            phoneNumberEv.Set();
        }
